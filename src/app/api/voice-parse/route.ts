...
function parseWithRegex(text: string): ParsedVoiceResult {
  ...
  // 1. Número do animal/paciente falado (ex: "animal 15", "paciente 14", "número 11", "animal 9")
  ...
  // Regra A: Palavra logo após a raça (Ex: "srd, princesa" ou "poodle, Bob" -> Princesa/Bob)
  ...
  return {
    ...
    breed,
    patient_name,
    weight_kg: weight_kg || 5.0,
    age: age || '1 ano',
    microchip,
    procedure_type,
    anesthesia_drugs: anesthesia_drugs.length > 0 ? anesthesia_drugs : (sex === 'F' ? ['P', 'I'] : ['P', 'K']),
    ...
    if (!apiKey) {
      if (transcriptText && transcriptText.trim().length > 1) {
        ...
        return NextResponse.json(
          { success: false, error: 'Nenhuma fala detectada. Por favor, fale mais perto do microfone ou digite a frase.' },
          { status: 400 }
        );
      }
    }
...
REGRAS:
1. "spoken_order_index": Se o veterinário disser "animal 15", "paciente 14", "número 10", extraia esse número inteiro aqui (ex: 15, 14, 10). Se não falar nenhum número, retorne null.
2. Nome do animal: Extraia o nome com precisão (ex: "SRD, princesa" -> nome: "Princesa", raça: "SRD").
3. "CAN" (cão/canino), "FEL" (gato/felino).
4. Sexo: "M" (macho), "F" (fêmea).
5. Raça padrão se não dita: "SRD".
6. Procedimento: "ORQ" (se falar "procedimento 1", "orquiectomia" ou macho), "OSH" (se falar "procedimento 2", "osh" ou fêmea), "OUTROS" (especiais).
...
10. Retorne apenas o JSON puro.`;
    ...
    return NextResponse.json(
      { success: false, error: 'Não foi possível captar o áudio com clareza. Por favor, tente novamente ou use a digitação rápida.' },
      { status: 400 }
    );
