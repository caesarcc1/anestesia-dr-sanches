...
function parseWithRegex(text: string): ParsedVoiceResult {
  ...
  // 1. Número do animal/paciente falado (ex: "animal 11", "paciente 14", "número 11", "animal 9")
  ...
  if (lower.includes('felin') || lower.includes('gato') || lower.includes('gata')) {
    species = 'FEL';
  } else if (lower.includes('canin') || lower.includes('cão') || lower.includes('cao') || lower.includes('cachorr') || lower.includes('cadela') || lower.includes('poodle') || lower.includes('pitbull')) {
    species = 'CAN';
  }
  ...
  // 7. Raça
  let breed = 'SRD';
  const knownBreeds = ['Pitbull', 'Poodle', 'Bulldog', 'Pinscher', 'Shih Tzu', 'Lhasa', 'Pastor', 'Labrador', 'Golden', 'Rottweiler', 'Dachshund', 'Siamês', 'Persa', 'Angorá', 'SRD', 'Vira-lata'];
  ...
  const blacklistWords = [
    ...
    'pitbull', 'poodle', 'srd', 'anos', 'quilos', 'kg', 'animal', 'paciente', 'de', 'do', 'da',
    'com', 'sem', 'propofol', 'propo', 'queta', 'quieta', 'quetamina', 'meloxicam', 'dipirona',
    'procedimento', 'osh', 'orq', 'castracao', 'castração', 'outros', 'pós', 'pos', 'anestesia', 'machi'
  ];

  // Regra A: Palavra logo após a raça (Ex: "poodle, Bob" ou "poodle Bob" -> Bob)
  ...
  if (breedFollowerMatch && breedFollowerMatch[1] && !blacklistWords.includes(breedFollowerMatch[1].toLowerCase()) && breedFollowerMatch[1].length > 1) {
    ...
    // Regra B: "de nome X", "chamado X", "nome X"
    const namePatterns = [
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!apiKey) {
      ...
      const result = parseWithRegex(transcriptText || 'Canino macho SRD 10kg Propofol Meloxicam');
...
REGRAS CRÍTICAS:
1. "spoken_order_index": Se o veterinário disser "animal 11", "paciente 14", "número 10", extraia esse número inteiro aqui (ex: 11, 14, 10). Se não falar nenhum número, retorne null.
2. Nome do animal: Extraia o nome do animal mesmo que venha antes/depois da raça (ex: "poodle, Bob" -> nome: "Bob", raça: "Poodle").
3. "CAN" (cão/canino), "FEL" (gato/felino).
4. Sexo: "M" (macho/machi), "F" (fêmea/femea).
5. Raça padrão se não dita: "SRD".
6. Procedimento: "ORQ" (se falar "procedimento 1", "orquiectomia" ou macho), "OSH" (se falar "procedimento 2", "osh" ou fêmea), "OUTROS" ("procedimento 3", etc.).
7. Fármacos: "P"(Propofol), "I"(Isoflurano), "K"(Quetamina/queta/quieta), "X"(Xilazina), "T"(Tramadol), "VK"(Vit K), "TM"(Transamin).
8. Pós: "A"(Agemoxi), "M"(Meloxicam), "D"(Dipirona).
9. "has_complication": true apenas se houver intercorrência clínica. Se disser "sem intercorrências" ou "tudo tranquilo", marque false.
10. Retorne apenas o JSON puro.`;
    ...
    // Se a IA em nuvem não responder, processa pelo analisador local com a transcrição informada
